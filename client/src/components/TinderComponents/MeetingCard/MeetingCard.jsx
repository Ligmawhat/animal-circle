  ...
  return (
    <div id="tindercard">
      {props.map(({ x, y, rot, scale }, i) => (
        <animated.div
          onClick={(e) => tinderDisLike(x.animation.fromValues[0], oneDog[i].id, isGone)}
          key={i}
          style={{ x, y }}
        >
          ...
          >
            <div className="insideCard">
              <p draggable={false}>{oneDog[i].name}</p>
              <div className="dogImage">
                <img draggable={false} src={`http://localhost:3001/dogs/${oneDog[i].url}`} />
              </div>
              <div
                draggable={false}
                style={{ overflowY: "scroll", height: "100px", width: "80% " }}
              >
                {oneDog[i].desc}
              </div>
            </div>
